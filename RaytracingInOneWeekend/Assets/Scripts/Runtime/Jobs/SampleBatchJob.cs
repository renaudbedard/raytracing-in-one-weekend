...
namespace Runtime.Jobs
{
	...
	unsafe struct SampleBatchJob : IJobParallelFor
	{
		...
		[ReadOnly] public uint2 SampleCountRange;
		...
		[ReadOnly] public NativeArray<float> InputSampleCountWeight;
		...
		[WriteOnly] public NativeArray<float> OutputSampleCountWeight;
		...
		public void Execute(int index)
		{
			...
			float sampleCountWeightAcc = InputSampleCountWeight[index];
			...
			uint samplesToAccumulate;
			float sampleCountWeight = sampleCountWeightAcc / sampleCount;
			if (sampleCountWeight == 0)
				samplesToAccumulate = SampleCountRange.x;
			else
			{
				float normalizedSampleCountWeight = saturate(unlerp(SampleCountWeightExtrema.x, SampleCountWeightExtrema.y, sampleCountWeight));
				samplesToAccumulate = (uint) round(lerp(SampleCountRange.x, SampleCountRange.y, normalizedSampleCountWeight));
			}

#if FULL_DIAGNOSTICS
			diagnostics.SampleCountWeight = sampleCountWeight;
#endif

			for (int s = 0; s < samplesToAccumulate; s++)
			{
				...
				if (Sample(eyeRay, ref rng,
					emissionStack, attenuationStack,
					ref nodeTraversalStack, ref hitCandidateStack, ref hitRecordList,
#if PATH_DEBUGGING
					doDebugPaths && s == 0,
#endif
					out float3 sampleColor, out float3 sampleNormal, out float3 sampleAlbedo, ref diagnostics, ref sampleCountWeightAcc))
				{
			...
			OutputSampleCountWeight[index] = sampleCountWeightAcc;
		...
		bool Sample(Ray eyeRay, ref RandomSource rng, float3* emissionStack, float3* attenuationStack,
			...
			bool doDebugPaths,
#endif
			out float3 sampleColor, out float3 sampleNormal, out float3 sampleAlbedo, ref Diagnostics diagnostics, ref float randomEventsAcc)
		{
			float3* emissionCursor = emissionStack;
			float3* attenuationCursor = attenuationStack;
			float randomEventsLocalAcc = 0;
			...
			for (; depth < TraceDepth; depth++)
			{
				...
				while (hitIndex < hitRecordBuffer.Length)
				{
					...
					*attenuationCursor++ = albedo;
					...
					randomEventsLocalAcc += rng.RandomEvents / pow(2, depth);
					rng.RandomEvents = 0;
			...
			randomEventsAcc += randomEventsLocalAcc; // * length(sampleColor);
